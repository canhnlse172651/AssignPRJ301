            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid ">
                        <h1 class="mt-4 px-5">Update product</h1>
                        <form id="registrationForm" action="MainServlet" method="post" onsubmit="return validateForm()">
                            <input type="hidden" name="productId" value="${ProUpdate.id}"/>
                            <div class="row px-5 mb-3 mt-4">
                                <div class="col-6">
                                    <label for="name" class="form-label">Product Name</label>
                                    <input type="text" class="form-control" id="name" name="name" value="${ProUpdate.name}" required>
                                </div>
                                <div class="col-6">
                                    <label for="categoryId" class="form-label">Category</label>
                                    <select class="form-control" id="categoryId" name="categoryId" required>
                                        <c:forEach var="category" items="${listCate}">
                                            <option value="${category.categoryId}" ${ProUpdate.categoryId ==category.categoryId?"selected":""}>${category.name}</option>
                                        </c:forEach>
                                    </select>  
                                </div>
                            </div>  
                            ...
                            <div class="row px-5 mb-3">
                                ...
                                <div class="col-3">
                                    ...
                                    <input type="number" class="form-control" id="quantity" name="quantity" value="${ProUpdate.stockQuantity}" required>
                                </div>
                                <div class="col-3">
                                    <label for="size" class="form-label">Size</label>
                                    <select class="form-control" id="size" name="size" required>
                                        <option value="Large" ${ProUpdate.size =="Large"?"selected":""}>Large</option>
                                        <option value="Medium" ${ProUpdate.size =="Medium"?"selected":""}>Medium</option>
                                        <option value="Small" ${ProUpdate.size =="Small"?"selected":""}>Small</option>
                                    </select>                                   
                                </div> 
                                <div class="col-3">
                                    <label for="status" class="form-label">Status</label>
                                    <select class="form-control" id="status" name="status" required>
                                        <option value="0" ${!ProUpdate.status ? "selected" : ""}>Deleted</option>
                                        <option value="1" ${ProUpdate.status ? "selected" : ""}>Active</option>
                                    </select>
                                </div>        
                            </div>  
                            <div class="row px-5 mb-3">
                                <label for="exampleFormControlTextarea1" class="form-label">Description</label>
                                <textarea class="form-control mx-2" id="exampleFormControlTextarea1" name="description" rows="5" required>${ProUpdate.description}</textarea>
                            </div>
                            <div class="row px-5 mb-3">
                                <div class="col-6">
                                    <label for="image" class="form-label">Category Image</label>
                                    <textarea class="form-control" id="image" name="image" rows="5" onchange="previewImage(event)" required>${ProUpdate.image}</textarea>
                                    <div class="row mx-1">
                                        <button type="submit" 
                                                class="btn btn-primary mt-3 col-2"
                                                name="btn" value="adminUpdateProduct">Save</button>
                                    </div>
                                </div>
                                <div class="col-6">
                                    <div id="imagePreview" style="padding: 10px; width: 100%; max-height: 200px;display: flex;
                                        ...
                                        <img id="preview" src="${ProUpdate.image}" style="width:auto ; max-height: 200px;object-fit: contain">
                                    </div>
                                </div>
                            </div>
                        </form>
